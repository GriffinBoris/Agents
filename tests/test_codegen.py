...
from agents.codegen.cli import main as cli_main
from agents.codegen.errors import SpecError
from agents.codegen.fields import parse_field
from agents.codegen.generators import available_targets, get_generator
from agents.codegen.generators.django.derive import admin_list_display, list_filter, raw_id_fields
from agents.codegen.generators.django.fields import render_declaration
from agents.codegen.generators.django.profile import build_django_profile
from agents.codegen.guidance_links import (
    GENERATORS_ROOT,
    drifted_links,
    linked_template_names,
    load_links,
)
from agents.codegen.naming import class_case, kebab_case, plural_snake, snake_case, title_case
from agents.codegen.profile import PROFILE_FILENAME, load_profile
from agents.codegen.resource import build_spec, load_spec
from agents.codegen.writer import MATCHES, WRITTEN, apply_files


ROOT = Path(__file__).resolve().parents[1]
EXAMPLES_ROOT = ROOT / 'agents' / 'codegen' / 'examples'
GOLDEN_ROOT = ROOT / 'tests' / 'golden' / 'codegen'
...
def generated_by_path(name: str = 'item', target: str = 'django') -> dict[str, str]:
    return {
        generated.path: generated.content for generated in get_generator(target).generate(load_example_spec(name))
    }
...
class SpecDerivationTest(unittest.TestCase):
    ...
    def test_admin_configuration_is_derived(self) -> None:
        self.assertEqual(
            ...
            admin_list_display(self.spec),
        )
        self.assertEqual(('name', 'code'), self.spec.search_fields)
        self.assertEqual(('status', 'catalog_entry'), list_filter(self.spec))
        self.assertEqual(('catalog_entry',), raw_id_fields(self.spec))

    def test_long_form_text_stays_out_of_admin_columns_and_search(self) -> None:
        self.assertNotIn('summary', admin_list_display(self.spec))
...
class GeneratedOutputTest(unittest.TestCase):
    ...
    def test_space_indent_profile_converts_leading_tabs(self) -> None:
        profile_text = (EXAMPLES_ROOT / '.codegen.yaml').read_text(encoding='utf-8')
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            views = next(item for item in get_generator('django').generate(spec) if item.path.endswith('views/item/views.py'))
...
class GuidanceLinkTest(unittest.TestCase):
    ...
    def test_every_template_is_governed_by_a_guidance_example(self) -> None:
        templates = {
            (path.parent.parent.name, path.name) for path in GENERATORS_ROOT.glob('*/templates/*.jinja')
        }
        self.assertEqual(
            set(),
...
class VueGeneratorTest(unittest.TestCase):
    """The second target proves the core is genuinely framework-neutral."""

    def setUp(self) -> None:
        self.spec = load_example_spec('item')
        self.generated = generated_by_path('item', 'vue')

    def paths(self) -> set[str]:
        return set(self.generated)

    def test_emits_the_frontend_contract_files(self) -> None:
        self.assertEqual(
            {
                'src/types/catalog/ItemInterface.ts',
                'src/types/catalog/ItemRequestInterface.ts',
                'src/utils/api.ts',
            },
            self.paths(),
        )

    def test_choices_become_a_typescript_enum(self) -> None:
        interface = self.generated['src/types/catalog/ItemInterface.ts']
        self.assertIn('export enum ItemStatus {', interface)
        self.assertIn('  ACTIVE = "ACTIVE",', interface)

    def test_fields_are_camel_cased_and_typed(self) -> None:
        interface = self.generated['src/types/catalog/ItemInterface.ts']
        self.assertIn('  sortOrder: number;', interface)
        self.assertIn('  catalogEntry: number;', interface)
        self.assertIn('  status: ItemStatus;', interface)
        self.assertIn('  createdTs: string;', interface)

    def test_request_schema_uses_zod_and_omits_backend_owned_fields(self) -> None:
        request = self.generated['src/types/catalog/ItemRequestInterface.ts']
        self.assertIn('export const itemInputSchema = z.object({', request)
        self.assertIn('  name: z.string().min(1, "Name is required"),', request)
        self.assertNotIn('catalogEntry:', request)
        self.assertNotIn('createdTs:', request)
        self.assertNotIn('  id:', request)

    def test_action_owned_state_is_not_editable_from_a_form(self) -> None:
        """archive owns status on the backend, so the form contract must not offer it."""
        request = self.generated['src/types/catalog/ItemRequestInterface.ts']
        self.assertEqual(('status',), self.spec.blocked_fields)
        self.assertNotIn('status:', request)

    def test_unused_enums_are_not_imported(self) -> None:
        """An action-blocked choice field leaves its enum out of the request contract."""
        request = self.generated['src/types/catalog/ItemRequestInterface.ts']
        self.assertNotIn('import { ItemStatus }', request)
        self.assertIn('export enum ItemStatus', self.generated['src/types/catalog/ItemInterface.ts'])

    def test_writable_choice_fields_do_import_their_enum(self) -> None:
        contact = generated_by_path('contact', 'vue')['src/types/contact/ContactRequestInterface.ts']
        self.assertIn('import { ContactStatus } from "./ContactInterface";', contact)
        self.assertIn('z.nativeEnum(ContactStatus)', contact)

    def test_defaults_factory_matches_the_schema(self) -> None:
        request = self.generated['src/types/catalog/ItemRequestInterface.ts']
        self.assertIn('export function createDefaultItemInput(): ItemRequestInterface {', request)
        self.assertIn('    name: "",', request)
        self.assertIn('    sortOrder: 0,', request)

    def test_api_methods_use_the_same_url_the_backend_routes(self) -> None:
        api = self.generated['src/utils/api.ts']
        backend = generated_by_path('item', 'django')['item/views/item/tests/test_views.py']

        self.assertIn(
            '`api/organizations/${organizationId}/workspaces/${workspaceId}'
            '/catalog-entries/${catalogEntryId}/items/list/`',
            api,
        )
        self.assertIn('/catalog-entries/{self.catalog_entry.id}/items/list/', backend)

    def test_actions_become_api_methods(self) -> None:
        self.assertIn('  archive: (', self.generated['src/utils/api.ts'])

    def test_typescript_uses_two_space_indentation(self) -> None:
        for path, content in self.generated.items():
            with self.subTest(path=path):
                self.assertNotIn('\t', content)

    def test_api_segment_is_a_merge_target(self) -> None:
        api = next(item for item in get_generator('vue').generate(self.spec) if item.path.endswith('api.ts'))
        self.assertTrue(api.merge)
        self.assertIn('items segment', api.note)


class TargetSelectionTest(unittest.TestCase):
    def test_target_flag_limits_generation(self) -> None:
        arguments = [str(EXAMPLES_ROOT / 'item.yaml'), '--profile', str(EXAMPLES_ROOT / '.codegen.yaml'), '--out']

        with tempfile.TemporaryDirectory() as temporary_directory:
            self.assertEqual(0, cli_main([*arguments, temporary_directory, '--target', 'vue']))
            written = {path.name for path in Path(temporary_directory).rglob('*') if path.is_file()}

        self.assertIn('ItemInterface.ts', written)
        self.assertNotIn('models.py', written)

    def test_unknown_target_is_rejected(self) -> None:
        with self.assertRaises(SystemExit):
            cli_main([str(EXAMPLES_ROOT / 'item.yaml'), '--target', 'nonsense'])

    def test_every_registered_target_declares_an_output_root(self) -> None:
        profile = load_profile(EXAMPLES_ROOT / '.codegen.yaml')

        for name in available_targets():
            with self.subTest(target=name):
                self.assertTrue(get_generator(name).output_root(profile))
...
class GoldenOutputTest(unittest.TestCase):
    ...
    def test_generated_output_matches_the_golden_files(self) -> None:
        ...
        for name in bundled_spec_names():
            for target in load_example_spec(name).targets:
                generated = generated_by_path(name, target)
                target_root = GOLDEN_ROOT / name / target

                for golden in sorted(path for path in target_root.rglob('*') if path.is_file()):
                    relative = golden.relative_to(target_root).as_posix()
                    with self.subTest(spec=name, target=target, path=relative):
                        self.assertIn(relative, generated)
                        self.assertEqual(
                            golden.read_text(encoding='utf-8'),
                            generated[relative],
                            f'{name}/{target}/{relative} drifted from the golden file',
                        )

    def test_golden_files_cover_every_non_empty_generated_file(self) -> None:
        for name in bundled_spec_names():
            for target in load_example_spec(name).targets:
                with self.subTest(spec=name, target=target):
                    generated = {
                        path for path, content in generated_by_path(name, target).items() if content.strip()
                    }
                    target_root = GOLDEN_ROOT / name / target
                    covered = {
    ...
    def test_every_bundled_spec_declares_every_target(self) -> None:
        for name in bundled_spec_names():
            with self.subTest(spec=name):
                self.assertEqual(set(available_targets()), set(load_example_spec(name).targets))
...
class CliTest(unittest.TestCase):
    def test_init_profile_writes_a_loadable_starter(self) -> None:
        ...
        django = build_django_profile(profile)
        self.assertIn('organization', profile.scopes)
        self.assertTrue(any(role.expect == 403 for role in django.roles))
        self.assertTrue(any(role.expect == 404 for role in django.roles))
        self.assertEqual(set(available_targets()), set(profile.targets))
    ...
    def test_starter_profile_generates_working_code(self) -> None:
        ...
        for generated in get_generator('django').generate(build_spec(spec, profile)):
    ...
    def test_scoped_queryset_lookup_scopes_before_fetching(self) -> None:
        ...
        views = next(
            item.content for item in get_generator('django').generate(build_spec(spec, profile)) if item.path.endswith('views.py')
        )
        self.assertEqual('scoped_queryset', build_django_profile(profile).detail_lookup)
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            exit_code = cli_main(
                [
                    ...
                    '--profile',
                    str(EXAMPLES_ROOT / '.codegen.yaml'),
                    '--out',
                    temporary_directory,
                    '--check',
                ]
...
class WriterTest(unittest.TestCase):
    def test_write_then_check_reports_no_drift(self) -> None:
        files = get_generator('django').generate(load_item_spec())
