...
- When reorganizing route code, use the repository's chosen route root rather than creating or extending a parallel feature-root folder.
- Add shared shell-level state beside the repository's existing shell state instead of introducing another global store location.
- Add transport helpers beside the canonical API client instead of introducing a second transport boundary.
