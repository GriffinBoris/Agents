...
from agents.codegen.generators.django.derive import (
    admin_list_display,
    feature_dir,
    history_log_fields,
    layout,
...
from agents.codegen.naming import python_tuple, title_case
...
def build_route(spec: ResourceSpec) -> Route:
    profile = build_django_profile(spec.profile)
    ...
    if parent and scope_field is not None:
        ...
        extra_imports.append((models_module(spec), spec.model))
...
def build_common_context(spec: ResourceSpec) -> dict[str, Any]:
    ...
    return {
        ...
        'history_log_fields': python_tuple(list(history_log_fields(spec))),
        'history_fields': history_log_fields(spec),
        'layout': layout(spec),
        'views_package': views_package(spec),
        'url_prefix': spec.url_prefix(),
        'permission': spec.permission or profile.default_permission,
    }
...
def build_model_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    ...
    path = f'{spec.app}/models/{spec.model}.py' if model_layout(spec) == 'package' else f'{spec.app}/models.py'
    return GeneratedFile(path=path, content=content, merge=model_layout(spec) != 'package')


def build_admin_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    context = {
        **common,
        'imports': [('django.contrib', 'admin'), (models_module(spec), spec.model)],
        'list_display': python_tuple(list(admin_list_display(spec))),
        ...
        'list_filter': python_tuple(list(list_filter(spec))) if list_filter(spec) else '',
        'raw_id_fields': python_tuple(list(raw_id_fields(spec))) if raw_id_fields(spec) else '',
    }
    content = render_template(environment, 'admin.py.jinja', context, build_django_profile(spec.profile).indent)
...
def build_serializers_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    ...
    context = {
        **common,
        'imports': [(models_module(spec), spec.model), ('rest_framework', 'serializers')],
        'input_fields': python_tuple(list(spec.input_fields), level=3),
        'output_fields': python_tuple(list(spec.output_fields), level=3),
        'scope_checks': scope_checks,
    }
    content = render_template(environment, 'serializers.py.jinja', context, build_django_profile(spec.profile).indent)
    return GeneratedFile(path=f'{feature_dir(spec)}/serializers.py', content=content)
...
def build_views_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    ...
    imports: list[tuple[str, str]] = [
        ...
        (serializers_module(spec), f'{spec.model}OutputSerializer'),
        *route.extra_imports,
    ]

    if 'create' in spec.endpoints or 'detail' in spec.endpoints:
        imports.append((serializers_module(spec), f'{spec.model}InputSerializer'))
    ...
    if spec.actions:
        imports.append((models_module(spec), spec.model))
    ...
    return GeneratedFile(path=f'{feature_dir(spec)}/views.py', content=content)


def build_urls_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    context = {**common, 'imports': [('django.urls', 'path'), (views_package(spec), 'views')]}
    content = render_template(environment, 'urls.py.jinja', context, build_django_profile(spec.profile).indent)
    return GeneratedFile(path=f'{feature_dir(spec)}/urls.py', content=content)


def build_app_urls_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    content = render_template(environment, 'app_urls.py.jinja', common, build_django_profile(spec.profile).indent)
...
def build_fixture_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    ...
    imports: list[tuple[str, str]] = [(models_module(spec), spec.model)]
...
def build_view_tests_file(spec: ResourceSpec, environment, common: dict) -> GeneratedFile:
    ...
    return GeneratedFile(path=f'{feature_dir(spec)}/tests/test_views.py', content=content)
    ...
    context = {
        **common,
        'imports': [
            (profile.fixtures_module, profile.fixtures_name),
            (models_module(spec), spec.model),
            (serializers_module(spec), f'{spec.model}InputSerializer'),
            (serializers_module(spec), f'{spec.model}OutputSerializer'),
        ],
    }
    content = render_template(environment, 'test_serializers.py.jinja', context, profile.indent)
    return GeneratedFile(path=f'{feature_dir(spec)}/tests/test_serializers.py', content=content)
    ...
    if layout(spec) != 'flat':
        paths.extend(
            [
                ...
                f'{feature_dir(spec)}/__init__.py',
                f'{feature_dir(spec)}/tests/__init__.py',
            ]
        )
...
class DjangoGenerator(BaseGenerator):
    """Emits the backend half of a resource: model, admin, serializers, views, URLs, tests."""

    name = 'django'
    templates_root = TEMPLATES_ROOT

    def output_root(self, profile: Profile) -> str:
        return build_django_profile(profile).backend_root
