...
def build_environment(templates_root: Path) -> Environment:
    """Each generator owns its own template directory and gets the shared filters."""
    ...
    environment.filters['camel'] = camel_case
    ...
    environment.filters['ts_object'] = ts_object
    environment.globals['render_imports'] = render_imports
    return environment


def render_imports(pairs: list[tuple[str, str]]) -> str:
    """Group (module, name) pairs into sorted, deduplicated Python import lines."""
...
def apply_indent(text: str, indent: str) -> str:
    """Templates are authored with tabs; convert leading tabs to whatever the target uses."""
    if indent == '\t':
        return text
